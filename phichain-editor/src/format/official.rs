        ...
        for line in phichain.lines {
            ...
            for note in notes {
                let kind = match note.kind {
                    crate::chart::note::NoteKind::Tap => NoteKind::Tap,
                    crate::chart::note::NoteKind::Drag => NoteKind::Drag,
                    crate::chart::note::NoteKind::Hold { .. } => NoteKind::Hold,
                    crate::chart::note::NoteKind::Flick => NoteKind::Flick,
                };

                let above = note.above;
                let speed = if matches!(note.kind, crate::chart::note::NoteKind::Hold { .. }) {
                    let mut speed = 0.0;
                    for event in &speed_events {
                        let value = event.evaluate(note.beat.value());
                        if let Some(value) = value {
                ...
                let note = Note {
                    ...
                    hold_time: match note.kind {
                        crate::chart::note::NoteKind::Hold { hold_beat } => {
